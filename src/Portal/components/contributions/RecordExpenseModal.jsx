...
const RecordExpenseModal = ({ onClose }) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        alert('Expense recorded: ' + JSON.stringify(formData));
        onClose();
    };
                ...
                <div style={{ padding: '0 1.5rem 1.5rem 1.5rem' }}>
                    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                        ...
                        <div style={{ position: 'relative' }}>
                            ...
                            <select name="expenseType" value={formData.expenseType} onChange={handleChange} style={{ ...inputStyle, appearance: 'none', cursor: 'pointer' }} required>
                                <option value="" style={{ color: 'var(--text-muted)', background: 'var(--surface-2)' }}>Select expense type...</option>
                                <option value="Utilities" style={{ color: 'var(--text-color)', background: 'var(--surface-2)' }}>Utilities</option>
                                <option value="Equipment" style={{ color: 'var(--text-color)', background: 'var(--surface-2)' }}>Equipment</option>
                                <option value="Maintenance" style={{ color: 'var(--text-color)', background: 'var(--surface-2)' }}>Maintenance</option>
                                <option value="Event" style={{ color: 'var(--text-color)', background: 'var(--surface-2)' }}>Event</option>
                                <option value="Other" style={{ color: 'var(--text-color)', background: 'var(--surface-2)' }}>Other</option>
                            </select>
                        ...
                        <div style={{ marginTop: '0.5rem' }}>
                            <button type="submit" style={{
                                ...
                                borderRadius: '0.5rem',
                                border: 'none',
                                background: '#f87171', // Matches the coral red from the screenshot
                                color: 'white',
                                fontWeight: '700',
                                cursor: 'pointer',
                                fontSize: '0.95rem'
